...
The GUI has multiple tabs: Target, Applis and Options.  The target tab contains
all properties related to a specific target like IP address, profiles of a
target if it exists and a short description of the selected target with
accompanying picture.  The Applis tab provides to the user an easy and
convenient way to choose the web application.  The options tab contains all
other options which are rarely modified.  
\medskip
...
\caption{target tab}
\end{figure}
\item[In the Applies tab]: (optional : welcome application is selected by
...
\end{figure}
\item[In the options tab]: (optional) For each option, you can select a
  disabled or enabled value. Options are initialized by default values.  Some
  applications need more configurations. So you have to press the configure
  button, in the “applis” tab, to display a new interface which contains one or
  more properties, each property combines a set of widgets representing the
  different constants. The widgets are initialized with the constants
  values. If selected applications don’t have any property, a dialog box
  appears informing you that no property exists for the selected
  applications. You can change values and press “validate”.  To finally launch
  the compilation, press the build button to compile, and the upload button to
  .  Ps: The user will be warned if one of the information entered is invalid.
\begin{figure}[H]
\centering
...
\end{description}
\subsection{Add or update properties}
...
user’s needs and options. The XML code must represent a description of the
widget that will be displayed. Widgets must be grouped by property, and each
property has a specific name. You can add one or more group of properties.  The
constant can be created during the SMEWS compilation, so in this case, you must
specify the default value in the description of the relative widget.
\begin{figure}[H]
\centering
...
\begin{table}[htb]
    \fbox{
      \colorbox{black!10}{
		\begin{minipage}{1\textwidth}
root of the xml code :\\
\\
{\tt <group label="toto>}

To enter a single line of text :\\
\\
{\tt <arg name="m1" type="int" show="Entry"/> }

Radio buttons to implement a one-of-many propositions "  \\
\\
{\tt <arg name="m4" show="Radiobutton">\\
	<input show="Radiobutton" selected="true" text="myradio1" Value="1"/>\\
	<input  show="Radiobutton" text="myradio2" Value="2"/>\\

</arg>}


Combobox to display a liste of propositions : \\
\\
{\tt <arg name="m2" state="readonly"  show="ComboBox">
\begin{tabbing}
...
a slider :\\
\\
{\tt <arg name="m7" from="0.5" to="100" orient="horizontal" resolution="0.1"
tickinterval="50" length="70m" show="Scale"/>
 }
...
\item[Emilien Hidden \textnormal{and} Olivier Szika] implemented the support
 ...
 \item[Rabab Bouziane \textnormal{and} Jomaa Narjes] wrote the graphical build interface.
\end{description}
