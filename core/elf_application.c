...
const struct elf_application_t const *all_applications __attribute__ ((section(".persistent_data"))) = 0;
...
#define APP_WRITE(var,val) do {						\
	const struct elf_application_t const *b = (val);		\
...
void elf_application_remove(const struct elf_application_t *application) {
	...
	const struct elf_application_t *appli = all_applications;
...
const struct output_handler_t *url_parse_step(uint8_t byte, unsigned char **url_blob, const struct output_handler_t **resources_index) {
    unsigned char *blob;
...
const struct output_handler_t *elf_application_parse_step(const struct connection *connection, uint8_t byte) {
    if(elf_application_get_count() > 0) {
	struct elf_application_parsing_t *parsing;
	const struct output_handler_t *output_handler = NULL;
	const struct elf_application_t *appli = all_applications;
